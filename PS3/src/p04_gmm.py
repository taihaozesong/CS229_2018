...
def main(is_semi_supervised, trial_num):
    ...
    sigma = []
    for i in range(K):
        temp_x = x[i * group_num:(i + 1) * group_num, :]
        mu[i, :] = np.mean(temp_x, axis=0)
        sigma.append((temp_x - mu[i, :]).T.dot(temp_x - mu[i, :]) / temp_x.shape[0])
...
def run_em(x, w, phi, mu, sigma):
    ...
    while it < max_iter and (prev_ll is None or np.abs(ll - prev_ll) >= eps):
        ...
        for i in range(K):
            sum_w = sum(w[:, i])
            mu[i] = x.T.dot(w[:, i]) / sum_w
            sigma[i] = (w[:, i][:, None] * (x - mu[i])).T.dot(x - mu[i]) / sum_w
